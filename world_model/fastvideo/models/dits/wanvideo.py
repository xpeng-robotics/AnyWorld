...
from fastvideo.models.controllers import SimpleAdapter, StateEmbedder
...
class WanImageEmbedding(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int):
        ...
        # Kaiming init for MLP (GELU ~ ReLU for init)
        for name, m in self.ff.named_modules():
...
class WanTransformer3DModel(CachableDiT):
    ...
    def __init__(self, config: WanVideoConfig, hf_config: dict[str,
        ...
        if config.add_control_adapter:
            self.control_adapter = SimpleAdapter(
                ...
                inner_dim,
                kernel_size=config.patch_size[1:],
                stride=config.patch_size[1:],
        ...
        if config.add_hand_controller:
            self.hand_control_adapter = StateEmbedder(
                config.data_statistics_path,
                config.in_dim_hand_control_adapter,
                config.hidden_size_hand_control_adapter,
            )
        else:
            self.hand_control_adapter = None

        if config.has_ref_conv:
            self.ref_conv = nn.Conv2d(16, inner_dim, kernel_size=(2, 2), stride=(2, 2))
            nn.init.kaiming_normal_(self.ref_conv.weight, mode="fan_out", nonlinearity="relu")
        else:
            self.ref_conv = None
    ...
    def forward(self,
                ...
                hand_states: torch.Tensor | None = None,
                guidance=None,
                **kwargs) -> torch.Tensor:
        ...
        if self.control_adapter is not None and control_camera_latents_input is not None:
            ...
            # y_camera shape: (b, c, f, h, w)
            # hidden_states shape: (b, c, f, h, w) before flatten
            # print("[CAMERA] wanvideo.py 708: Loading camera control adapter")
            hidden_states = hidden_states + y_camera

        if self.hand_control_adapter is not None and hand_states is not None:
            # hidden_states shape: (b, c, f, h, w) torch.Size([1, 5120, 21, 30, 52])
            # encoder_hidden_states shape torch.Size([b, 512, 4096])
            y_hand = self.hand_control_adapter(hand_states).transpose(1, 2)  # (b, c, f)
            y_hand = y_hand.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 1, post_patch_height, post_patch_width)  # (b, c, f, h, w)
            hidden_states = hidden_states + y_hand
        
        hidden_states = hidden_states.flatten(2).transpose(1, 2)

        # Video control
        if self.ref_conv is not None and reference_latents is not None:
            if len(reference_latents.shape) == 5:
                reference_latents = reference_latents[:, :, 0]
            reference_latents = self.ref_conv(reference_latents).flatten(2).transpose(1, 2)
            hidden_states = torch.concat([reference_latents, hidden_states], dim=1)
            # num_frames += 1
        ...
        if reference_latents is not None:
            hidden_states = hidden_states[:, reference_latents.shape[1]:]
            num_frames -= 1
            post_patch_num_frames = num_frames // p_t
